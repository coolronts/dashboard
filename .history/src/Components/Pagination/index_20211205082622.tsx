...
const Pagination: React.FC<props> = ({ items, showItems, siblings = 1 }) => {
  ...
  const styles = {
    ...
    row: "flex bg-red-300 items-center h-full text-xl font-sans font-semibold px-2",
    numberRows: "flex w-full items-center bg-blue-300",
    currentPage: "bg-blue-400 text-white",
    hellip: "px-2",
  };
  let pageIncrementBtn = null;
  if (totalPage > maxPageNumberLimit) {
    pageIncrementBtn = <p className={styles.hellip}> &hellip; </p>;
  }
  let pageDecrementBtn = null;
  if (minPageNumberLimit >= 1) {
    pageDecrementBtn = <p className={styles.hellip}> &hellip; </p>;
  }
