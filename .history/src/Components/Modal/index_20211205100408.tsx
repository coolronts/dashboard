...
const Modal: React.FC<ModalProps> = ({ children, handleModal }) => {
  const styles = {
    background:
      "fixed w-full h-full overflow-auto bg-gray-900 opacity-70 top-0 left-0 z-10",
    container: "absolute w-full h-full bg-black",
    body: "absolute bg-white absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 p-24 rounded-2xl shadow-2xl ",
    close:
