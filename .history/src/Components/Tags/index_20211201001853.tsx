...
const Tags: React.FC<dataProps> = ({
  ...
  const styles = {
    body: `flex justify-center items-center ring-2 rounded-md w-36 text-center py-1 text-xs font-bold font-sans capitalize
    ${
    ...
    ${
      type && type !== undefined && type.toLowerCase() === "done"
        ? "text-green-500"
        : type === "pending"
        ? "text-yellow-500"
  ...
  return (
    <div className={styles.body}>
      {type && type.toLowerCase() === "done" && (
        <Done className="text-xl font-bold p-1 rounded-full bg-green-200 text-green-800" />
      )}
