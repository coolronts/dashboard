          ...
          <div className="px-6 mb-3">
            <div className="flex justify-between text-md text-gray-400 pt-1 items-center">
              ...
              <div className="flex opacity-50">
                {days.map((item, i) => <p key={i} onClick={() =>handleDays(i)} className= {`${optionsDays===i ? 'bg-gray-100':'bg-transparent' } px-2 py-1 ring-1 ring-gray-300 cursor-pointer hover:shadow-lg shadow  text-gray-700 ml-4 rounded text-md font-semibold font-sans tracking-tight`}>{item}</p>)}
              </div>
            </div>
          </div>
          <div className=" h-1/3">
